...
#这个模型类的作用是将输入的序列数据经过 LSTM 单元处理后，得到下一时刻的预测结果。
class ConvLSTM(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):    #num_layers是num_hidden的长度
    ...
    def forward(self, frames, mask_true):
        ...
        for i in range(self.num_layers):
            ...
            h_t.append(zeros)
            c_t.append(zeros)
            # print("num_layers_i:",i)
        # 循环处理每一帧
        for t in range(self.configs.total_length - 1):#total_lenth = 15
            # print("t:",t)
            if t < self.configs.input_length:   #input_length = 5
                net = frames[:,t]
            else:
            ...
            #在循环神经网络（RNN）中，h_t和c_t通常分别表示隐藏状态（hidden state）和细胞状态（cell state）。
            # 这两个状态是RNN单元内部的重要组成部分，用于捕捉序列中的信息和记忆。
            # 循环处理后续 LSTMCell
            for i in range(1, self.num_layers):
                # print('layer number is:',str(i),net.shape,h_t[i].shape)###########
                h_t[i], c_t[i] = self.cell_list[i](h_t[i - 1], h_t[i], c_t[i])
                # print(str(i),'h_t:',h_t[i].shape,"c_t:",c_t[i].shape)##################
            # 使用最后一个 LSTMCell 的输出进行卷积
            x_gen = self.conv_last(h_t[self.num_layers-1])###最后一层是一个二维卷积
            next_frames.append(x_gen)
            # print("next_frames_1:")
        # [length, batch, channel, height, width] -> [batch, length, height, width, channel]
        next_frames = torch.stack(next_frames, dim=0).permute(1, 0, 3, 4, 2).contiguous()
        # next_frames = torch.stack(next_frames, dim=1).permute(1, 0, 3, 4, 2).contiguous()
        # print("next_frames_2:", next_frames.shape)
        # print("next_frames_3:",next_frames.shape)##############
        return next_frames

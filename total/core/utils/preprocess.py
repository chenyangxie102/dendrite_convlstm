...
# 这段程序的作用是将输入的图像张量重新排列成"patch"形式的张量。
# "Patch"是指将图像分割成小块的过程，每个小块称为一个"patch"。
# 这个过程通常用于图像处理中的局部操作或特征提取。
...
# 这段函数的作用与前一个函数相反,将经过reshape_patch函数处理的图像"patch"张量重新转换回原始的图像张量形式。
